...
export function computeTraceWaveformDebug(trace, options = {}) {
    ...
    if (points.length < 2) {
        return {
            ...
            minPeakDistanceSamples: Math.max(2, Math.round((trace.sampleRate || 1) * (options.minPeakDistanceSec ?? 0.35))),
        };
    }
    ...
    const minPeakDistanceSamples = Math.max(2, Math.round((trace.sampleRate || 1) * (options.minPeakDistanceSec ?? 0.35)));
    const peaks = [];
    ...
    const durationSec = trace.durationSec ??
...
export function computeWaveformPeriodicityProfile(data, sampleRate, minBpm = BPM_MIN, maxBpm = BPM_MAX, stepBpm = 1) {
    const n = data.length;
    ...
    if (!Number.isFinite(energy) || energy <= 1e-9)
        return null;
    ...
    const confidence = clamp(topMass *
        1.6 *
        (1 - normalizedEntropy * 0.75) *
        clamp(contrast / 0.1, 0.05, 1), 0, 1);
    const scoreByLag = new Map();
    for (const candidate of candidatesWithPosterior) {
        scoreByLag.set(candidate.lag, clamp(candidate.posterior, 0.01, 1));
    }
    const scores = lags.map((lag) => Number((scoreByLag.get(lag) ?? 0.01).toFixed(6)));
    return {
        minBpm,
        maxBpm,
        stepBpm,
