const MODES = ["half", "fundamental", "double"];
...
export class BpmBayesTracker {
    ...
    update(measurements, dtSec, context) {
        ...
        for (const measurement of measurements) {
            ...
            const reliability = clamp(this.sourceReliability[measurement.source] *
                confidence *
                (0.5 + quality * 0.5), 0.1, 1.5);
            const harmonicConfusion = clamp(this.sourceHarmonicConfusion[measurement.source] +
                motion * 0.15 +
                snrPenalty * 0.05, 0.05, 0.85);
            const sigma = clamp(2.5 + (1 - confidence) * 11 + motion * 8 + snrPenalty * 2, 2, 24);
            for (let bpmIndex = 0; bpmIndex < this.bpmGrid.length; bpmIndex++) {
                ...
                for (let modeIndex = 0; modeIndex < MODES.length; modeIndex++) {
                    ...
                    const blended = (1 - harmonicConfusion) * modeObs +
                        harmonicConfusion *
        ...
        this.applyWaveformEvidence(context.waveformProfile ?? null, motion, quality, snrPenalty, measurements, context.referenceBpm);
        if (context.referenceBpm != null &&
            Number.isFinite(context.referenceBpm)) {
            this.observeReference(context.referenceBpm, context.referenceStrength ?? 1);
        }
        this.normalize();
    ...
    observeReference(referenceBpm, strength = 1, _measurements = []) {
        ...
        for (let bpmIndex = 0; bpmIndex < this.bpmGrid.length; bpmIndex++) {
            const referenceLikelihood = gaussian(referenceBpm, this.bpmGrid[bpmIndex], sigma);
            for (let modeIndex = 0; modeIndex < MODES.length; modeIndex++) {
                const posteriorIndex = this.toIndex(bpmIndex, modeIndex);
    ...
    reinforceReference(referenceBpm, measurements, strength = 1, updatedAtTs = Date.now(), waveformProfile = null) {
        ...
        for (const mode of MODES) {
            this.harmonicPrior[mode] = clamp(this.harmonicPrior[mode] * (1 - s * 0.45) +
                modeWeights[mode] * (s * 0.45), 0.65, 1.9);
        }
        ...
        this.referencePriorWeight = clamp(Math.max(this.referencePriorWeight * 0.9, 0.18) + s * 0.45, 0, 1);
        ...
        this.observeReference(referenceBpm, clamp(0.25 + s * 0.2, 0.25, 0.5), measurements);
    }
    reinforceHarmonicReference(referenceBpm, measurements, strength = 1, updatedAtTs = Date.now(), waveformProfile = null) {
        ...
        const modeWeights = this.inferReferenceModeWeights(referenceBpm, measurements);
        for (const mode of MODES) {
            this.harmonicPrior[mode] = clamp(this.harmonicPrior[mode] * (1 - s * 0.55) +
                modeWeights[mode] * (s * 0.55), 0.65, 1.9);
        }
        this.referencePriorLastUpdatedTs = Number.isFinite(updatedAtTs)
            ? updatedAtTs
            : Date.now();
    ...
    updateReliability(referenceBpm, measurements) {
        ...
        for (const measurement of measurements) {
            ...
            const aliasErr = Math.min(Math.abs(measurement.bpm - referenceBpm * 0.5), Math.abs(measurement.bpm - referenceBpm * 2));
            const reliabilityTarget = clamp(1 - directErr / 28, 0, 1);
            const confusionTarget = aliasErr + 2 < directErr ? 1 : 0;
            this.sourceReliability[measurement.source] = clamp(this.sourceReliability[measurement.source] * (1 - learningRate) +
                reliabilityTarget * learningRate, 0.2, 1.2);
            this.sourceHarmonicConfusion[measurement.source] = clamp(this.sourceHarmonicConfusion[measurement.source] *
                (1 - learningRate) +
                confusionTarget * learningRate, 0.05, 0.9);
        }
    }
    getSnapshot() {
        return {
    ...
    loadSnapshot(snapshot) {
        ...
        else if (this.referencePriorOrigin === "snapshot_restore" ||
            this.referencePriorOrigin === "none") {
            this.referencePriorLastUpdatedTs = null;
        }
            ...
            for (let prevBpm = 0; prevBpm < this.bpmGrid.length; prevBpm++) {
                ...
                for (let nextMode = 0; nextMode < MODES.length; nextMode++) {
                    ...
                    for (let nextBpm = 0; nextBpm < this.bpmGrid.length; nextBpm++) {
                        const kernel = gaussian(this.bpmGrid[nextBpm], this.bpmGrid[prevBpm], sigmaBpm);
    ...
    applyWaveformEvidence(waveformProfile, motion, quality, snrPenalty, measurements, referenceBpm) {
        ...
        const agreement = this.estimateWaveformAgreement(waveformProfile, measurements, referenceBpm);
        const gain = clamp((0.015 +
        ...
        const scoreAtObservedBpm = (observedBpm) => {
            if (!Number.isFinite(observedBpm) ||
                observedBpm < waveformProfile.minBpm ||
                observedBpm > waveformProfile.maxBpm) {
                return 0.03;
            }
            const index = Math.round((observedBpm - waveformProfile.minBpm) / waveformProfile.stepBpm);
    ...
    updateWaveformReliability(referenceBpm, waveformProfile) {
        ...
        const scoreAt = (bpm) => {
            ...
            const index = clamp(Math.round((bpm - waveformProfile.minBpm) / waveformProfile.stepBpm), 0, waveformProfile.scores.length - 1);
            const score = waveformProfile.scores[index];
            return Number.isFinite(score) ? clamp(score, 0.01, 1) : 0.01;
        };
        const harmonicSupport = Math.max(scoreAt(referenceBpm), scoreAt(referenceBpm * 0.5) * 0.95, scoreAt(referenceBpm * 2) * 0.7);
        const topCandidate = waveformProfile.topCandidates?.[0] ?? null;
        const topErr = topCandidate
            ? Math.min(Math.abs(topCandidate.bpm - referenceBpm), Math.abs(topCandidate.bpm - referenceBpm * 0.5), Math.abs(topCandidate.bpm - referenceBpm * 2))
            : 999;
        const offTargetPenalty = topCandidate && topErr > 14 ? clamp(topCandidate.posterior, 0, 1) : 0;
        const targetReliability = clamp(harmonicSupport * 1.35 - offTargetPenalty * 0.7, 0.05, 0.85);
        const learningRate = clamp(0.08 + waveformProfile.confidence * 0.12, 0.08, 0.2);
        this.waveformReliability = clamp(this.waveformReliability * (1 - learningRate) +
            targetReliability * learningRate, 0.05, 0.9);
    }
    ...
    applyPersistentReferencePrior(dtSec) {
        if (this.referencePriorBpm == null ||
        ...
        for (let bpmIndex = 0; bpmIndex < this.bpmGrid.length; bpmIndex++) {
            const fundamental = this.bpmGrid[bpmIndex];
            const referenceLikelihood = gaussian(this.referencePriorBpm, fundamental, sigma);
            for (let modeIndex = 0; modeIndex < MODES.length; modeIndex++) {
                const mode = MODES[modeIndex];
                const posteriorIndex = this.toIndex(bpmIndex, modeIndex);
    ...
    inferReferenceModeWeights(referenceBpm, measurements) {
        ...
        const valid = measurements.filter((measurement) => measurement.bpm != null &&
            Number.isFinite(measurement.bpm) &&
            measurement.confidence > 0);
        if (!valid.length) {
            return {
