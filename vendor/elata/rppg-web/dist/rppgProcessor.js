...
import { ChannelGainController, ChromPulseModel, computeSignalSnrDb, } from "./rppgSignalModel.js";
const BPM_MIN = 40;
const BPM_MAX = 180;
...
export class RppgProcessor {
    constructor(backend, sampleRate = 30, windowSec = 5) {
        this.backend = backend;
        this.samples = [];
        this.bpmHistory = [];
        this.cameraCalibration = new MuseCalibrationModel();
        this.fusion = new MuseFusionCalibrator();
        this.bayesTracker = new BpmBayesTracker(BPM_MIN, BPM_MAX);
...
function analyzeWindow(samples) {
    ...
    const respiration = respirationEstimate &&
...
function estimateDominantBpm(data, sampleRate, minHz, maxHz) {
    ...
    if (!(bestMag > 0) || !Number.isFinite(bestBpm))
        return null;
...
function calculateBpmViaAutocorrelation(data, fps, bpmHint) {
    ...
    if (bestLag <= 0)
        return null;
    const build = (lag) => {
        if (lag < minLag || lag > maxLag || lag >= n)
            return null;
        const score = lagScorer(lag);
