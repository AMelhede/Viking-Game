export function normalizeRppgError(error, diagnostics) {
    const sessionError = coerceSessionError(error ?? diagnostics?.lastError);
    const backendUnavailable = !sessionError && diagnostics?.backendMode === "unavailable";
    if (!sessionError && !backendUnavailable)
        return null;
    if (backendUnavailable) {
        return {
            ...
            detail: "Packaged WASM assets were unavailable, so the session cannot provide full estimation.",
            guidance: "Serve the packaged WASM assets or pass wasmJsUrl, wasmBinaryUrl, or wasmImporter explicitly.",
            retryable: false,
            terminal: false,
        };
    }
    ...
    const phase = sessionError && sessionError.code === "processor_error"
        ? "runtime"
        : phaseFromDiagnostics(diagnostics?.state?.phase ??
            (sessionError?.code === "capture_error" ? "runtime" : "startup"));
    if (sessionError?.code === "processor_error" ||
        ...
        return {
            ...
            detail,
            guidance: "Dispose the failed session and recreate it. Use createManagedRppgSession() if you want automatic retries.",
            retryable: true,
            terminal: true,
        };
    }
    if (lower.includes("2d context unavailable") ||
        ...
        return {
            ...
            detail,
            guidance: "Confirm the browser supports 2D canvas capture for the current video surface, then retry.",
            retryable: false,
            terminal: false,
        };
    }
    ...
    if (sessionError?.code === "face_mesh_init_failed") {
        return {
            ...
            detail,
            guidance: "Use faceMesh: 'off' for center-box mode, or ensure FaceMesh assets load successfully.",
            retryable: false,
            terminal: false,
        };
    }
    if (sessionError?.code === "capture_error") {
        if (lower.includes("never started playing") ||
            ...
            return {
                ...
                detail,
                guidance: "Confirm the video element is attached to the stream, autoplay is allowed, and playback has started before creating the session.",
                retryable: true,
                terminal: false,
            };
        }
        return {
            ...
            detail,
            guidance: "Check camera permissions, video playback state, and browser capture support before retrying.",
            retryable: true,
            terminal: false,
        };
    }
...
function phaseFromDiagnostics(phase) {
    return phase === "runtime" ? "runtime" : "startup";
}
function coerceSessionError(value) {
